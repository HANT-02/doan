...
export default function LessonSummaryEditor({
    lessonId,
    title = 'Tổng kết buổi học',
    subtitle = 'Ghi lại nội dung dạy, phản hồi lớp và bài tập sau buổi học.',
}: LessonSummaryEditorProps) {
    const { data, isLoading, isFetching, error, refetch } = useGetLessonSummaryQuery(lessonId, {
        skip: !lessonId,
    });
    const [upsertSummary, { isLoading: isSaving }] = useUpsertLessonSummaryMutation();
    ...
    const handleSave = async () => {
        try {
            ...
            toast.success('Đã lưu tổng kết buổi học.');
            refetch();
        } catch (err) {
        ...
        <Paper variant="outlined" sx={{ p: 3, borderRadius: 3 }}>
            <Stack spacing={2.5}>
                ...
                <Stack direction="row" spacing={1} flexWrap="wrap" useFlexGap>
                    <Chip size="small" variant="outlined" label={data?.data?.summary ? 'Đã có summary' : 'Chưa có summary'} />
                    <Chip size="small" color="primary" variant="outlined" label={`${completionCount}/5 trường chính đã điền`} />
                </Stack>
