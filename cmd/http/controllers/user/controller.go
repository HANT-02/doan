...
type Controller interface {
	...
	ConfirmChangePasswordOTP(ctx *gin.Context)
	VerifyOTP(ctx *gin.Context)
	GetMe(ctx *gin.Context)
}

// RegisterRoutesV1 register routes for version 1
func RegisterRoutesV1(router *gin.RouterGroup, controller Controller, manager config.Manager) {
	...
	protected := v1.Group("")
	protected.Use(middleware.AuthMiddleware(manager))
	{
		protected.POST("/change-password", controller.ChangePassword)
		protected.POST("/change-password/confirm-otp", controller.ConfirmChangePasswordOTP)
